...
class MergejilView(APIView):
    ...
    def get(self, request):
        ...
        if mergejil_id:
            ...
            mergejils = Mergejil.objects.all()
    ...
    def put(self, request):
        mergejil_id = request.data.get('mergejil_ID')
        mergejil = Mergejil.objects.filter(mergejil_ID=mergejil_id).first()
        if not mergejil:
            return Response({"error": "Mergejil not found"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def delete(self, request):
        ...
        mergejil.delete()
        return Response({"message": "Mergejil deleted successfully"}, status=status.HTTP_204_NO_CONTENT)
